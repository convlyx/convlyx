...
- Bulk import students from a spreadsheet ("Importar alunos", admin/secretary). Four-step wizard on `/students`: upload `.xlsx`/`.xls`/`.csv` (parsed in-browser via lazy-loaded SheetJS) → map the file's columns to Name/Email/Phone/Category via auto-detected dropdowns (no rigid template, no drag-and-drop) → editable preview table with live per-row validation (invalid/duplicate email, missing name/category) that the importer fixes inline → per-row results (created / reactivated / skipped / failed). A batch "default category" fills rows without a recognized category column; existing emails are flagged (ACTIVE → skipped, INACTIVE → reactivated). Reuses the single-create invite+DB path (`createUserAccount`); each imported student gets an invite email. Rows imported sequentially server-side; one bad row never aborts the batch.
...
- Class lifecycle status is **derived from the clock on read** (`effectiveClassStatus` in `src/lib/constants/class.ts`), used by all UI gates + analytics: the stored `status` column only persists `CANCELLED` (plus an early `IN_PROGRESS` on theory check-in). This makes status correct with zero lag and **without depending on any background job**. A Supabase `pg_cron` job (`scripts/sync-class-statuses.cron.sql`) may *also* keep the stored column current for any raw-SQL/reporting consumers, but the app no longer relies on it — if that job is disabled or unprovisioned, the app is unaffected.
...
- **Operational actions** (`admin.ops.*`, all audited, confirm dialogs): suspend / reactivate a tenant (type-the-name confirmation), rename a tenant, edit school settings (name, cancellation-notice hours, practical self-enroll — **never** timezone), activate / deactivate any staff membership, and **reset a staff member's password** (mints a short-lived Supabase recovery link the operator copies + relays — SMTP-independent). **Suspend is enforced**: an INACTIVE tenant is denied at both the tRPC layer (`protectedProcedure`, via the tenant status folded into the per-request membership load) and the pages layer (`getDashboardUser`).
- **Support view** (`admin.support.*`, read-only): from a school's account page, "Ver alunos" opens an audited student list (search + paginate) and a read-only student detail (profile, courses, enrolments, exams). Plus **cross-tenant user lookup** (`/platform-admin/lookup`): find any user by email and see every membership (group, school, role, status, tenant suspended?, last-active) — support triage for "why can't this login get in". These are the only admin surfaces that return individual PII — **every access writes an `AuditLog` row** (`student.list_view` / `student.view_detail` / `user.lookup`), the writes are **fail-closed** (a view aborts if its audit row can't be written), and each page shows a "your access is logged" banner. Impersonation was deliberately descoped to this console-rendered read-only view (auth-path risk); see `docs/superpowers/specs/2026-07-13-admin-panel-internal-tools-design.md` §7.
...
- **Audit log viewer** (`/platform-admin/audit`): operator action trail (actor email, action, target, timestamp, metadata). Filter by action, actor, **target id** (click any row's target to pivot), and **date range** (7/30/90d); offset-paginated. `AuditLog` is intentionally not tenant-scoped.
