...
- **Middleware:** `/novidades` is added to the root-domain allowlist (otherwise apex 404s it). The blog is **root-domain only**: a tenant subdomain hitting `/novidades*` gets a 308 redirect to the canonical root URL (prevents broken footer links + duplicate content; runs before auth so logged-out visitors redirect too). Accordingly, the in-app popover links open the **public root blog in a new tab** — the root origin is derived client-side from the current host (`computeRootOrigin` in `novidades-button.tsx`), resolving to `convlyx.com` in prod and `localhost:3000` in dev.
- **SEO:** posts are auto-listed in `sitemap.xml`; `robots.ts` already disallows tenant subdomains and points crawlers to the root sitemap; every page carries a `canonical` to `convlyx.com`. No per-post Search Console action needed.
