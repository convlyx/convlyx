...
- **Rolled out:** list pages (`/students`, `/instructors`, `/staff`, `/enrollments`, `/schools`), detail pages (`/classes/[id]`, `/students/[id]`, `/instructors/[id]`), `/analytics`, and the role-based home panels (student / instructor / admin).
- **Date-based panels:** the home panels' class lists derive date ranges from `new Date()`, which the server can't byte-match. Resolved by computing the ranges once on the server in Portugal time (`src/lib/dashboard-ranges.ts`) and passing them to the panels as props, so prefetch and the client `useQuery` share the identical input.
- **Still skipped:** `/calendar` — its range follows the user-driven calendar viewport (month navigation), so there's no fixed server input to prefetch.
- Side fixes: `CreateUserDialog`/`EditUserDialog` fetch `school.list` only when open (`enabled: open`); the class-detail student roster loads only when the enroll picker opens (`enabled: showAddStudent`); list pages and the enrollments tabs use `keepPreviousData` (+ inactive-tab prefetch on enrollments) so page/tab changes don't flash a skeleton.
