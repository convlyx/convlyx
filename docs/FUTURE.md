...
- [ ] **`StudentPicker` server-side search** — the picker in `create-class-dialog.tsx` and `class-detail-view.tsx` fetches ALL active students unbounded (`user.list` with no page/pageSize), then filters by category + search text client-side. **Decision (2026-05-31): leave as-is** — client-side filtering gives instant UX with no per-keystroke round-trips at typical school size (dozens–low hundreds), which is better than a debounced server query at this scale. **Trigger to revisit**: a tenant approaching ~1000 students (also the cap on `user.list`'s Supabase auth-status merge, see `user.ts`). Then move StudentPicker to its own paginated/debounced server search, adapting the category filter and "select all visible up to max" logic which currently assume the full list is in memory.
