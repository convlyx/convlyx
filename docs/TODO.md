...
- [~] **DPA + recorded acceptance** — DPA page (`/contrato-de-subcontratacao`, Art. 28 draft) incorporated into the Terms; acceptance recorded via `ConsentRecord` + a first-login gate (blocking DPA modal for admins, non-blocking banner for users). See `docs/superpowers/specs/2026-07-01-consent-and-dpa-recording-design.md`. **Remaining: (a)** legal sign-off of the DPA wording (currently ships flagged "carece de revisão jurídica"); **(b)** once signed off, **bump `LEGAL_VERSIONS.dpa` in `src/lib/legal.ts`** — that re-prompts every school/user to accept the FINAL version (acceptances collected now are against the draft).
