...
- [x] **E2E tests (Playwright)** — v1 ships with auth-as-seeded-admin (also acts as login regression), enrol student, mark attendance, and cancel class. Lives in `e2e/`, runs via `pnpm e2e` against the local dev server. Test data is created via raw `pg` (Prisma 7's ESM-only client can't be required from Playwright's CJS loader) and cleaned up per-test. Local-only for now — CI integration is a separate follow-up (needs a Supabase test project + workflow secrets). Follow-up paths to add: create-class (date picker + recurrence), notifications surface check.
