...
export function StudentHome({ userName }: { userName: string }) {
  ...
  function getTimeUntil(date: Date): string {
    ...
    if (hours > 24) {
      ...
      return t("dashboard.timeInDays", { days });
    }
    if (hours > 0) return t("dashboard.timeInHours", { hours, minutes });
    ...
    <div className="space-y-6">
      ...
      {nextClass ? (
        ...
        <div className="rounded-2xl border-2 border-dashed border-muted-foreground/20 p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">{t("dashboard.noScheduledClasses")}</p>
          <Link href="/classes" className="text-sm text-primary font-medium mt-1 inline-block">
            {t("dashboard.exploreClasses")}
          </Link>
        </div>
      )}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1.5 space-y-1 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>·</span>
                      <span>{cls._count.enrollments}/{cls.capacity} {t("dashboard.spots")}</span>
                    </div>
                  </div>
                </div>
              </div>
