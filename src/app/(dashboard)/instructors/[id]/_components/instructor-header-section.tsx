...
export function InstructorHeaderSection({ id, userRole }: Props) {
  ...
  const { data: instructor, isLoading } = trpc.user.instructorHeader.useQuery({ id });
        ...
        <div className="flex-1 min-w-0 space-y-2">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2 sm:shrink-0 self-start">
              {canManage && !instructor.anonymized && (
                <Button
                  variant="outline"
                  size="sm"
                  className="gap-1.5"
              ...
              {canManage && !instructor.anonymized && (instructor.status === "ACTIVE" ? (
                <Button
                  variant="destructive"
                  size="sm"
                  className="gap-1.5"
...
function InstructorHeaderSkeleton() {
  return (
